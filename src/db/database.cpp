...
Ascent* Database::getAscentAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID ascentID		= VALID_ITEM_ID(row->at(ascentsTable->primaryKeyColumn		->getIndex()), ItemTypeAscent);
	QString	title				=				row->at(ascentsTable->titleColumn			->getIndex()).toString();
	ItemID	peakID				= ItemID(		row->at(ascentsTable->peakIDColumn			->getIndex()), ItemTypePeak);
	...
	ItemID	tripID				= ItemID(		row->at(ascentsTable->tripIDColumn			->getIndex()), ItemTypeTrip);
...
Peak* Database::getPeakAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID peakID	= VALID_ITEM_ID(row->at(peaksTable->primaryKeyColumn->getIndex()), ItemTypePeak);
	...
	ItemID	regionID	= ItemID(		row->at(peaksTable->regionIDColumn	->getIndex()).toInt(), ItemTypeRegion);
...
Trip* Database::getTripAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID tripID	= VALID_ITEM_ID(row->at(tripsTable->primaryKeyColumn	->getIndex()), ItemTypeTrip);
...
Hiker* Database::getHikerAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID hikerID	= VALID_ITEM_ID(row->at(hikersTable->primaryKeyColumn	->getIndex()), ItemTypeHiker);
...
Region* Database::getRegionAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID regionID	= VALID_ITEM_ID(row->at(regionsTable->primaryKeyColumn	->getIndex()), ItemTypeRegion);
	QString	name			=				row->at(regionsTable->nameColumn		->getIndex()).toString();
	ItemID	rangeID			= ItemID(		row->at(regionsTable->rangeIDColumn		->getIndex()).toInt(), ItemTypeRange);
	ItemID	countryID		= ItemID(		row->at(regionsTable->countryIDColumn	->getIndex()).toInt(), ItemTypeCountry);
...
Range* Database::getRangeAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID rangeID	= VALID_ITEM_ID(row->at(rangesTable->primaryKeyColumn	->getIndex()), ItemTypeRange);
...
Country* Database::getCountryAt(BufferRowIndex rowIndex) const
{
	...
	ValidItemID countryID	= VALID_ITEM_ID(row->at(countriesTable->primaryKeyColumn->getIndex()), ItemTypeCountry);
	...
	for (Table* candidateTable : tables) {
		...
		// Look for references in normal table
			...
			for (const Column* otherTableColumn : candidateNormalTable->getColumnList()) {
				...
				for (const ValidItemID& primaryKey : primaryKeys) {
					QList<BufferRowIndex> rowIndexList = candidateNormalTable->getMatchingBufferRowIndices(otherTableColumn, ID_GET(primaryKey, table->itemType));
			...
			// EXECUTE
				...
				for (const auto& [affectedColumn, rowIndices] : affectedCells) {
					...
					for (const BufferRowIndex& rowIndex : rowIndices) {
						ValidItemID primaryKey = VALID_ITEM_ID(affectedPrimaryKeyColumn->getValueAt(rowIndex), affectedPrimaryKeyColumn->table->itemType);
						// Remove single instance of reference to the key about to be removed
						candidateNormalTable->updateCell(parent, primaryKey, affectedColumn, QVariant());
					}
				}
			}
		}
