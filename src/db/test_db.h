...
const auto CREATE_ASCENTS		= QLatin1String(R"(CREATE TABLE Ascents(ascentID INTEGER PRIMARY KEY, title NVARCHAR, peakID INTEGER REFERENCES Peaks(peakID), date DATE, perDayIndex INTEGER NOT NULL, time TIME, kind INT NOT NULL, traverse BINARY NOT NULL, difficultySystem INTEGER, difficultyGrade INTEGER, tripID INTEGER REFERENCES Trips(tripID), notes NVARCHAR))");
const auto CREATE_PEAKS			= QLatin1String(R"(CREATE TABLE Peaks(peakID INTEGER PRIMARY KEY, name NVARCHAR NOT NULL, height INTEGER, volcano BINARY NOT NULL, regionID INTEGER REFERENCES Regions(regionID), mapsLink NVARCHAR, earthLink NVARCHAR, wikiLink NVARCHAR))");
const auto CREATE_TRIPS			= QLatin1String(R"(CREATE TABLE Trips(tripID INTEGER PRIMARY KEY, name NVARCHAR NOT NULL, startDate DATE, endDate DATE, notes NVARCHAR))");
const auto CREATE_HIKERS		= QLatin1String(R"(CREATE TABLE Hikers(hikerID INTEGER PRIMARY KEY, name NVARCHAR NOT NULL))");
const auto CREATE_REGIONS		= QLatin1String(R"(CREATE TABLE Regions(regionID INTEGER PRIMARY KEY, name NVARCHAR NOT NULL, rangeID INTEGER REFERENCES Ranges(rangeID), countryID INTEGER REFERENCES Countries(countryID)))");
const auto CREATE_RANGES		= QLatin1String(R"(CREATE TABLE Ranges(rangeID INTEGER PRIMARY KEY, name NVARCHAR NOT NULL, continent INT))");
const auto CREATE_COUNTRIES		= QLatin1String(R"(CREATE TABLE Countries(countryID INTEGER PRIMARY KEY, name NVARCHAR NOT NULL))");
const auto CREATE_PHOTOS		= QLatin1String(R"(CREATE TABLE Photos(photoID INTEGER PRIMARY KEY, ascentID INTEGER REFERENCES Ascents(ascentID), localIndex INTEGER NOT NULL, filepath NVARCHAR NOT NULL))");
const auto CREATE_PARTICIPATED	= QLatin1String(R"(CREATE TABLE Participated(ascentID INTEGER NOT NULL, hikerID INTEGER NOT NULL, CONSTRAINT participatedPK PRIMARY KEY (ascentID, hikerID)))");

const auto INSERT_ASCENT		= QLatin1String(R"(INSERT INTO Ascents(title, peakID, date, perDayIndex, time, kind, traverse, difficultySystem, difficultyGrade, tripID, notes) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?))");
const auto INSERT_PEAK			= QLatin1String(R"(INSERT INTO Peaks(name, height, volcano, regionID, mapsLink, earthLink, wikiLink) values(?, ?, ?, ?, ?, ?, ?))");
const auto INSERT_TRIP			= QLatin1String(R"(INSERT INTO Trips(name, startDate, endDate, notes) values(?, ?, ?, ?))");
const auto INSERT_HIKER			= QLatin1String(R"(INSERT INTO Hikers(name) values(?))");
const auto INSERT_REGION		= QLatin1String(R"(INSERT INTO Regions(name, rangeID, countryID) values(?, ?, ?))");
const auto INSERT_RANGE			= QLatin1String(R"(INSERT INTO Ranges(name, continent) values(?, ?))");
const auto INSERT_COUNTRY		= QLatin1String(R"(INSERT INTO Countries(name) values(?))");
const auto INSERT_PHOTO			= QLatin1String(R"(INSERT INTO Photos(ascentID, index, filepath) values(?, ?, ?))");
const auto INSERT_PARTICIPATED	= QLatin1String(R"(INSERT INTO Trips(ascentID, hikerID) values(?, ?))");



int addAscent(QSqlQuery& q, const QString& title, int peakID, const QDate& date, int perDayIndex, const QTime& time, int kind, bool traverse, int difficultySystem, int difficultyGrade, int tripID, const QString& notes)
{
	q.addBindValue(title);
	q.addBindValue(peakID);
	q.addBindValue(date);
	q.addBindValue(perDayIndex);
	q.addBindValue(time);
	q.addBindValue(kind);
