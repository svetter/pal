...
void FilterWizardFoldOpPage::initializePage()
{
	...
	if (columnToUse->type == Integer) {
		explainLabel->setText(tr("The column you chose can contain multiple values for each row in the filtered table. A filter can work on a list of those values, their count, or on their maximum, minimum, sum, or average."));
	} else {
		explainLabel->setText(tr("The column you chose can contain multiple values for each row in the filtered table. A filter can work on a list of those values, or on their count."));
	}
...
{
	FilterFoldOp selectedFoldOp = getSelectedFoldOp();
...
FilterWizard::FilterWizard(QWidget* parent, Database& db) :
	QWizard(parent),
	tablePage(FilterWizardTablePage(parent, db.ascentsTable)),
	columnPage(FilterWizardColumnPage(parent, db.ascentsTable, tablePage)),
	foldOpPage(FilterWizardFoldOpPage(parent, db.ascentsTable, columnPage)),
	numberPrefPage(FilterWizardNumberPrefPage(parent)),
	namePage(FilterWizardNamePage(parent, db.ascentsTable, tablePage, columnPage, foldOpPage))
{
	setModal(true);
